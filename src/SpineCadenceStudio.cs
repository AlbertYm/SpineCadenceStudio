...
static class Engine {
 ...
 static object[] SampleColors(Dictionary<string,object>[] keys,string type,Dictionary<string,object> slot,List<double> times){
  ...
  for(int i=0;i<keys.Length;i++){if(i>0)Check(Num(keys[i],"time")>Num(keys[i-1],"time"),"颜色关键帧时间未严格递增。");if(!keys[i].ContainsKey("curve")||Convert.ToString(keys[i]["curve"])=="stepped")continue;var curve=keys[i]["curve"] as object[];Check(curve!=null&&curve.Length==setup.Length*4,"颜色曲线控制点格式错误。");foreach(var point in curve){double v=Convert.ToDouble(point);Check(!double.IsNaN(v)&&!double.IsInfinity(v),"颜色曲线包含非有限数值。");}}
 ...
 static double[] ExpandDeform(Dictionary<string,object> frame,int length){var result=new double[length];int offset=(int)Num(frame,"offset");Check(offset>=0,"负数变形偏移。");if(frame.ContainsKey("vertices")){var values=Arr(frame["vertices"]);Check(offset+values.Length<=length,"变形偏移越界。");for(int i=0;i<values.Length;i++)result[offset+i]=Convert.ToDouble(values[i]);}return result;}
 static void VerifyAnimation(object expected,object actual,string path){
  var d=expected as Dictionary<string,object>;if(d!=null){var a=actual as Dictionary<string,object>;Check(a!=null,"回读结构不一致："+path);if(path.Contains("/deform/")){int length=Math.Max((int)Num(d,"offset")+(d.ContainsKey("vertices")?Arr(d["vertices"]).Length:0),(int)Num(a,"offset")+(a.ContainsKey("vertices")?Arr(a["vertices"]).Length:0));var ev=ExpandDeform(d,length);var av=ExpandDeform(a,length);for(int i=0;i<length;i++)Check(Math.Abs(ev[i]-av[i])<0.0051,"回读变形数值不一致："+path);}
foreach(var pair in d){if(path.Contains("/deform/")&&(pair.Key=="vertices"||pair.Key=="offset"))continue;if(pair.Key=="curve"&&!a.ContainsKey(pair.Key))continue;object value;if(!a.TryGetValue(pair.Key,out value)){if(pair.Value==null)continue;if(pair.Value is double||pair.Value is int||pair.Value is decimal){double def=pair.Key!="time"&&path.Contains("/scale")?1:0;Check(Math.Abs(Convert.ToDouble(pair.Value)-def)<0.0051,"回读缺失数值："+path+"/"+pair.Key);continue;}if(pair.Value is object[]&&((object[])pair.Value).Length==0)continue;throw new Exception("回读缺失字段："+path+"/"+pair.Key);}VerifyAnimation(pair.Value,value,path+"/"+pair.Key);}return;}
 ...
 public static Dictionary<string,object> ConvertData(Dictionary<string,object> data,int fps){
  ...
  foreach(var entry in animations){var anim=Map(entry.Value);double end=Duration(anim);Check(end*fps<200000,"动画过长，采样点超过安全上限。");var times=new List<double>();for(int i=0;i<Math.Ceiling(end*fps);i++)if(i/(double)fps<end)times.Add(i/(double)fps);times.Add(end);
   foreach(var group in anim.Keys.ToArray()){
    Check(group=="bones"||group=="slots"||group=="events"||group=="drawOrder"||group=="attachments","不支持的动画通道："+group+"；未生成工程。");
    if(group=="events")continue;
    if(group=="attachments"){
     foreach(var skin in Map(anim[group]).Values)foreach(var slot in Map(skin).Values)foreach(var attachment in Map(slot).Values){var timelines=Map(attachment);foreach(var kind in timelines.Keys.ToArray()){Check(kind=="deform","暂不支持附件通道："+kind);var frames=Arr(timelines[kind]).Select(Map).ToArray();Check(frames.Length>0,"空变形通道。");int length=frames.Max(f=>(int)Num(f,"offset")+(f.ContainsKey("vertices")?Arr(f["vertices"]).Length:0));Check(length<=100000&&times.Count*(long)Math.Max(1,length)<20000000,"变形采样超出安全上限。");var values=frames.Select(f=>ExpandDeform(f,length)).ToArray();for(int j=0;j<frames.Length;j++){if(j>0)Check(Num(frames[j],"time")>Num(frames[j-1],"time"),"变形时间未严格递增。");if(frames[j].ContainsKey("curve"))Check(Convert.ToString(frames[j]["curve"])=="stepped"||(frames[j]["curve"] is object[]&&Arr(frames[j]["curve"]).Length==4),"变形曲线格式错误。");}
     timelines[kind]=times.Select(t=>{int j=Array.FindLastIndex(frames,f=>Num(f,"time")<=t+1e-9);var v=j<0?new double[length]:(double[])values[j].Clone();if(j>=0&&j+1<frames.Length){double ratio=0;if(!frames[j].ContainsKey("curve"))ratio=(t-Num(frames[j],"time"))/(Num(frames[j+1],"time")-Num(frames[j],"time"));else if(frames[j]["curve"] is object[])ratio=CurveValue(t,Num(frames[j],"time"),0,Num(frames[j+1],"time"),1,Arr(frames[j]["curve"]),0);for(int k=0;k<length;k++)v[k]+=(values[j+1][k]-v[k])*ratio;}var result=Key(t);result["curve"]="stepped";int first=Array.FindIndex(v,x=>x!=0),last=Array.FindLastIndex(v,x=>x!=0);if(first>=0){if(first>0)result["offset"]=first;result["vertices"]=v.Skip(first).Take(last-first+1).Cast<object>().ToArray();}return (object)result;}).ToArray();}}
     continue;
    }
 ...
 public static string ConvertFile(string input,string outputRoot,int fps,string exe,Action<string> status,bool exportBundle=false,string packPreset=null,string resourceRoot=null){
...
class MainForm:Form{
 string packPreset=null; CheckBox exportBundle=new CheckBox(); ListView list=new ListView(); TextBox resourceOutput=new TextBox(),output=new TextBox(),spine=new TextBox(),log=new TextBox();RadioButton beside=new RadioButton(),custom=new RadioButton();NumericUpDown fps=new NumericUpDown();Label state=new Label();StatusProgress progress=new StatusProgress();RoundButton start,stop;bool busy,cancel;Panel settings;FlowLayoutPanel toolbar;Stopwatch elapsed=new Stopwatch();Timer clock=new Timer();string stage="就绪";int finished,total;
 public MainForm(string[] paths){Text="Spine Cadence Studio "+Engine.Version+" | 动画降帧";Font=new Font("Microsoft YaHei UI",10);BackColor=Color.FromArgb(17,20,29);ForeColor=Color.FromArgb(237,243,255);ClientSize=new Size(1100,760);MinimumSize=new Size(940,690);AutoScaleMode=AutoScaleMode.Dpi;AllowDrop=true;
  var layout=new TableLayoutPanel{Dock=DockStyle.Fill,Padding=new Padding(22),ColumnCount=1,RowCount=7};layout.RowStyles.Add(new RowStyle(SizeType.Absolute,58));layout.RowStyles.Add(new RowStyle(SizeType.Absolute,48));layout.RowStyles.Add(new RowStyle(SizeType.Percent,100));layout.RowStyles.Add(new RowStyle(SizeType.Absolute,280));layout.RowStyles.Add(new RowStyle(SizeType.Absolute,88));layout.RowStyles.Add(new RowStyle(SizeType.Absolute,25));layout.RowStyles.Add(new RowStyle(SizeType.Absolute,48));Controls.Add(layout);
  ...
  settings=new Panel{Dock=DockStyle.Fill,Padding=new Padding(0,12,0,0)};layout.Controls.Add(settings,0,3);var grid=new TableLayoutPanel{Dock=DockStyle.Fill,ColumnCount=3,RowCount=6};grid.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,155));grid.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,100));grid.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,130));for(int i=0;i<6;i++)grid.RowStyles.Add(new RowStyle(SizeType.Absolute,42));settings.Controls.Add(grid);
  ...
  grid.Controls.Add(Label("导出设置"),0,3);exportBundle.Text="转换后同时导出 .skel + .atlas + PNG";exportBundle.AutoSize=true;exportBundle.Checked=false;grid.Controls.Add(exportBundle,1,3);grid.Controls.Add(Button("打包预设…",()=>{using(var d=new OpenFileDialog{Filter="Spine 打包设置|*.json"})if(d.ShowDialog()==DialogResult.OK){packPreset=d.FileName;exportBundle.Checked=true;log.AppendText("打包预设："+packPreset+Environment.NewLine);}}),2,3);grid.Controls.Add(Label("游戏资源目录"),0,5);resourceOutput.Dock=DockStyle.Fill;resourceOutput.Enabled=false;grid.Controls.Add(resourceOutput,1,5);var resourceBrowse=Button("选择资源目录…",()=>{using(var d=new FolderBrowserDialog{Description="选择资源总目录，每个工程自动新建同名子文件夹"})if(d.ShowDialog()==DialogResult.OK)resourceOutput.Text=d.SelectedPath;});resourceBrowse.Enabled=false;grid.Controls.Add(resourceBrowse,2,5);exportBundle.CheckedChanged+=(sender,evt)=>{resourceOutput.Enabled=exportBundle.Checked;resourceBrowse.Enabled=exportBundle.Checked;};var hint=new ToolTip();hint.SetToolTip(resourceOutput,"留空：工程结果中的游戏资源子目录；指定：总目录/原工程名，重名自动加序号");spine.Text=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles),"Spine","Spine.com");spine.Dock=DockStyle.Fill;grid.Controls.Add(spine,1,4);grid.Controls.Add(Button("选择程序…",()=>{using(var d=new OpenFileDialog{Filter="Spine 命令行|Spine.com"})if(d.ShowDialog()==DialogResult.OK)spine.Text=d.FileName;}),2,4);
  ...
  foreach(var box in new[]{output,spine,resourceOutput}){box.BackColor=Color.FromArgb(32,36,48);box.ForeColor=ForeColor;box.BorderStyle=BorderStyle.FixedSingle;}fps.BackColor=Color.FromArgb(32,36,48);fps.ForeColor=ForeColor;
 }
...
static class Program{
 [STAThread] static int Main(string[] args){try{if(args.Length>0&&args[0]=="--convert"){string result=Engine.ConvertFile(args[1],args[2],int.Parse(args[3]),args[4],s=>Console.WriteLine(s),args.Length>5&&args[5]=="--export",null,args.Length>6?args[6]:null);Console.WriteLine(result);return 0;}Application.EnableVisualStyles();Application.SetCompatibleTextRenderingDefault(false);Application.Run(new MainForm(args));return 0;}catch(Exception e){if(args.Length>0&&args[0]=="--convert")Console.Error.WriteLine(e.Message);else MessageBox.Show(e.Message,"Spine Cadence Studio");return 1;}}
}
}
